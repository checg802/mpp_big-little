...
static HI_VOID SAMPLE_IVE_Ann_Mlp_Predict(SAMPLE_IVE_ANN_INFO_S* pstAnnInfo)
{
    ...
    for (i = 0; i < pstAnnInfo->stSrc.u16Height; ++i)
    {
		
        ps16q16Src = (HI_S16Q16*)(pstAnnInfo->stSrc.pu8VirAddr + pstAnnInfo->stSrc.u16Stride * i);
        ...
        SAMPLE_IVE_Ann_Mlp_BinFeature(pstAnnInfo->stGray.pu8VirAddr[0],\
										pstAnnInfo->stGray.u16Width, \
                                      pstAnnInfo->stGray.u16Height, \
	...
	if (HI_TRUE == bIsSuccess)
	{
		...
		for (i = 0; i < pstAnnInfo->stDst.u16Height; i++)
		{			
			ps16q16Dst = (HI_S16Q16*)(pstAnnInfo->stDst.pu8VirAddr + pstAnnInfo->stDst.u16Stride * i);
			s16q16Response = 0;
			s32ResponseCls = -1;
...
HI_VOID SAMPLE_IVE_Ann(HI_VOID)
{
    ...
    //predict
    SAMPLE_IVE_Ann_Mlp_Predict(&s_stAnnInfo);
    //uninit
