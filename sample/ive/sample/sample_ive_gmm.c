...
HI_VOID SAMPLE_IVE_Gmm(HI_CHAR chEncode, HI_CHAR chVo)
{
	...
	HI_BOOL bEncode = chEncode ? HI_TRUE : HI_FALSE;
	...
	******************************************/
	 ...
	 ******************************************/
	 ...
	 if (HI_TRUE == bEncode)
	 {
		 ...
		 s32Ret = SAMPLE_COMM_VENC_GetGopAttr(VENC_GOPMODE_NORMALP,&stGopAttr,enVideoNorm);
		 SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, END_GMM_5, 
			 "SAMPLE_COMM_VENC_GetGopAttr failed,Error(%#x)!\n",s32Ret);  
		  s32Ret = SAMPLE_COMM_VENC_Start(s_VeH264Chn, enStreamType,enVideoNorm,aenSize[0],enRcMode,0,&stGopAttr);
		  SAMPLE_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, END_GMM_5, 
