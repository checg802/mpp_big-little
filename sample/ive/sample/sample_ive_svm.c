...
static HI_VOID SAMPLE_IVE_Svm_Predict(SAMPLE_IVE_SVM_INFO_S* pstSvmInfo)
{
    ...
    for ( i = 0; i < pstSvmInfo->stSrc.u16Height/*4*/; i++)
    {
		ps16q16Src = (HI_S16Q16 *)(pstSvmInfo->stSrc.pu8VirAddr  + i * pstSvmInfo->stSrc.u16Stride);
        ...
        SAMPLE_IVE_Svm_Feature(pstSvmInfo->stGrayImg.pu8VirAddr[0], pstSvmInfo->stGrayImg.u16Width, \
                               pstSvmInfo->stGrayImg.u16Height, ps16q16Src);
    }
	...
	for (i = 0; i < pstSvmInfo->stVote.u16Height; i++)
	{
		pu16Vote = (HI_U16*)(pstSvmInfo->stVote.pu8VirAddr + i * pstSvmInfo->stVote.u16Stride);
		u8MaxVoteIdx = 0;
...
HI_VOID SAMPLE_IVE_Svm(HI_VOID)
{
    ...
    //Predict
    SAMPLE_IVE_Svm_Predict(&s_stSvmInfo);
    //Uninit
