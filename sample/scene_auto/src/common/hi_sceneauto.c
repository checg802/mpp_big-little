...
HI_S32 Sceneauto_SetVpssParam(VPSS_GRP VpssGrp, HI_SCENEAUTO_3DNRPARAM_S st3dnrParam)
{
	...
	if (HI_SUCCESS != s32Ret)
    {
        ...
        free(stVpssGrpNrS.stNRSParam_V4.stVideoNrAuto.pastNRSParam);
        free(stVpssGrpNrS.stNRSParam_V4.stVideoNrAuto.pau32ISO);
        return HI_FAILURE;
    }
...
void *SceneAuto_DrcThread(void* pVoid)
{
    ...
    prctl(PR_SET_NAME, (unsigned long)"SceneAutoDrcThread", 0, 0, 0);
