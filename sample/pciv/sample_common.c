...
HI_S32 SAMPLE_StopAllVi(HI_VOID)
{
    ...
    for (i = 0; i < 16; i++)
    {
		...
		if(HI_SUCCESS != s32Ret)
		{
			 printf("HI_MPI_VI_DisableChn Error,s32Ret:%d\n",s32Ret);
		}
    ...
    for (i = 0; i < 4; i++)
    {
        s32Ret = HI_MPI_VI_DisableDev(i*2);
		if(HI_SUCCESS != s32Ret)
		{
			 printf("HI_MPI_VI_DisableDev Error,s32Ret:%d\n",s32Ret);
		}
    }
...
HI_S32 SAMPLE_StopAllVo(HI_VOID)
{
    ...
    for (VoDev = 0; VoDev < VO_MAX_DEV_NUM; VoDev++)
    {
        for (VoChn = 0; VoChn < VO_MAX_CHN_NUM; VoChn++)
        {
		   ...
		   if(HI_SUCCESS != s32Ret)
		   {
				printf("HI_MPI_VO_DisableChn Error,s32Ret:%d\n",s32Ret);
		   }
        }
		...
		if(HI_SUCCESS != s32Ret)
		{
			printf("HI_MPI_VO_DisableVideoLayer Error,s32Ret:%d\n",s32Ret);
		}
        s32Ret = HI_MPI_VO_Disable(VoDev);
		if(HI_SUCCESS != s32Ret)
		{
			printf("HI_MPI_VO_Disable Error,s32Ret:%d\n",s32Ret);
		}
    }
...
HI_S32 SAMPLE_StartVenc(HI_U32 u32GrpCnt, HI_BOOL bHaveMinor,PAYLOAD_TYPE_E aenType[2], PIC_SIZE_E aenSize[2],VIDEO_NORM_E enCodeMode) /**/	//l00346266 
{
    ...
    for (i=0; i<u32GrpCnt; i++)
    {
        VeGroup = i;
        VeChn = i;
		s32Ret = SAMPLE_COMM_VENC_GetGopAttr(VENC_GOPMODE_NORMALP,&stGopAttr,enCodeMode);
		if (s32Ret != HI_SUCCESS)
        {
            printf("\nSAMPLE_COMM_VENC_GetGopAttr failed! err 0x%x\n", s32Ret);
            return HI_FAILURE;
        }
		s32Ret = SAMPLE_COMM_VENC_Start(VeChn,aenType[0],enCodeMode,aenSize[0],SAMPLE_RC_CBR,0,&stGopAttr);
        if (s32Ret != HI_SUCCESS)
        {
...
HI_S32 Payload2FilePostfix(PAYLOAD_TYPE_E enPayload, HI_U8* szFilePostfix)
{
    ...
    {
        strncpy((char*)szFilePostfix, ".h264", 10);
    }
    else if (PT_JPEG == enPayload)
    {
        strncpy((char*)szFilePostfix, ".jpg", 10);
    }
    else if (PT_MJPEG == enPayload)
    {
        strncpy((char*)szFilePostfix, ".mjp", 10);
    }
    else
    {
...
HI_VOID* SampleGetVencStreamProc(HI_VOID *p)
{
    ...
    for (i = VeChnStart; i < VeChnStart + s32ChnTotal; i++)
    {
        ...
        snprintf(aszFileName[i], 64 , "stream_%s%d%s", "chn", i, szFilePostfix);
