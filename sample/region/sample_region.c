...
static HI_S32 SAMPLE_RGN_SYS_Init(HI_VOID)
{
    ...
    struVbConf.astCommPool[2].u32BlkCnt  = 20;

    stSize.u32Width  = 768;
    stSize.u32Height = 576;
    u32BlkSize = SAMPLE_COMM_SYS_CalcPicVbBlkSize2(&stSize, 
                    SAMPLE_PIXEL_FORMAT, SAMPLE_SYS_ALIGN_WIDTH);
    struVbConf.astCommPool[3].u32BlkSize = u32BlkSize;
...
HI_VOID SAMPLE_RGN_VoIntf_Usage(HI_VOID)
{
    printf("intf:\n");
...
HI_S32 SAMPLE_RGN_StartVenc(VENC_CHN VencChn)
{
	...
	stChnAttr.stGopAttr.stNormalP.s32IPQpDelta = 1;
...
#define HI_IO_NOBLOCK 1
    ...
    if (HI_SUCCESS == s32Ret)
    {
		...
		if(HI_SUCCESS != s32Ret)	
		{
			printf("VENC_ReleaseStream Failed,s32Ret:0x%x\n",s32Ret);
			HI_ASSERT(0);
		}
    }
    ...
    while (HI_FALSE == bExit || i < pstVencThread->s32FrmCnt)
    {
        ...
        if (stChnStat.u32CurPacks > 0 || stChnStat.u32LeftStreamBytes > 0)
        {
            ...
            else if (bArea)
            {
                ...
                s32Ret = SAMPLE_RGN_DestroyRegion(Handle, stRgnsize.u32RgnNum);
				if (HI_SUCCESS != s32Ret)
			    {
					SAMPLE_RGN_NOT_PASS(s32Ret);
			    }
...
HI_S32 SAMPLE_RGN_AddCoverToVpss(HI_VOID)
{
    ...
    HI_U32 u32VpssChnNum = VPSS_MAX_PHY_CHN_NUM;
...
HI_S32 SAMPLE_RGN_AddOsdToVpss(HI_VOID)
{
    ...
    BITMAP_S stBitmap;
    HI_S32 s32VpssGrpNum;
    HI_S32 u32VpssChnNum = VPSS_MAX_PHY_CHN_NUM;
...
HI_S32 SAMPLE_RGN_AddOsdToVencAndVpss(HI_VOID)
{
    ...
    stVencOsdReverseInfo.VpssChn                 = VpssChn;
     ...
     *************************************************/
    char pfilename[30];
	snprintf(pfilename,sizeof(pfilename),"%s","venc_encode_stream.h264");
    pastream = fopen(pfilename, "wb");
    HI_ASSERT( NULL != pastream);
