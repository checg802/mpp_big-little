...
static inline HI_VOID Useage(HI_VOID)
{
	...
	#ifndef __HuaweiLite__
    printf("usage: ./Isp debug [IspDev] [ispdepth] [aedepth] [awbdepth] [defogdepth]. sample: ./isp_debug 0 10 10 10 10\n");
	#else
	printf("usage: Isp debug [IspDev] [ispdepth] [aedepth] [awbdepth] [defogdepth]. sample: isp_debug 0 10 10 10 10\n");
	#endif
    printf("[ispDev]  ----------Isp device number\n");
...
HI_S32 ISP_DEBUG_Exit(HI_VOID)
{
    ...
    s32Ret = HI_MPI_ISP_SetDebug(g_IspDev, &stDebug);
    ...
    stDebug.u32Depth = 0;
    stDebug.u32PhyAddr = 0;         
    s32Ret = ISP_DehazeCtrl(g_IspDev, DEHAZE_DEBUG_ATTR_SET, (HI_VOID*)&stDebug);
    if (HI_SUCCESS != s32Ret)
    {
...
#endif
{
    ...
    if (argc > 1)
    {
        g_IspDev = atoi(argv[1]);/* IspDev */
        if ((g_IspDev != 0) && (g_IspDev != 1))
        {
            Useage();
            return HI_FAILURE;
        }
    ...
    if (argc > 2)
    {
        au32Depth[0] = atoi(argv[2]);/* the frame depth */
        if (0 == au32Depth[0])
        {
            Useage();
            return HI_FAILURE;
    ...
    if (argc > 3)
    {
        au32Depth[1] = atoi(argv[3]);/* the frame depth */
        if (0 == au32Depth[1])
        {
            Useage();
            return HI_FAILURE;
    ...
    if (argc > 4)
    {
        au32Depth[2] = atoi(argv[4]);/* the frame depth */
        if (0 == au32Depth[2])
        {
            Useage();
            return HI_FAILURE;
    ...
    if (argc > 5)
    {
        au32Depth[3] = atoi(argv[5]);/* the frame depth */
        if (0 == au32Depth[3])
        {
            Useage();
            return HI_FAILURE;
    ...
    for (i = 0; i < argc - 2; i++)
    {
        ...
        else
        {
            if (3 == i)
            {
               s32Ret = ISP_DehazeCtrl(g_IspDev, DEHAZE_DEBUG_WAIT_END, HI_NULL);
               if (HI_SUCCESS != s32Ret)
               {   
