...
3、DIS debug使用注意事项：
	（1）DIS debug只支持2轴DIS，且在抓取2轴DIS debug的时候要在串口执行：
		echo 1113 > /proc/sys/dev/debug/proc_message_enable
	（2）当需要在用vi_dump时，同时把dis_debug信息保存成文件时，需要把vi_dump工具中的宏DIS_DATA_DEBUG打开。
